...
# Rolling‐window GMV backtest including Glasso, Sample, JM, Equal, Cov–Correlation shrinkage, and Graphical Elastic Net (GELNET)
...
# --- GELNET parameters ---
lambda_gelnet <- 1.1    # penalty parameter (tune between ~0.05 and 0.2 if desired)
...
weights_gmv     <- matrix(NA, ncol = n_assets, nrow = 0)
weights_scov    <- matrix(NA, ncol = n_assets, nrow = 0)
weights_gmv_jm  <- matrix(NA, ncol = n_assets, nrow = 0)
weights_ccov    <- matrix(NA, ncol = n_assets, nrow = 0)
weights_gelnet  <- matrix(NA, ncol = n_assets, nrow = 0)

colnames(weights_gmv)    <- colnames(weights_scov) <-
  colnames(weights_gmv_jm) <- colnames(weights_ccov) <-
  colnames(weights_gelnet) <- colnames(X_all)
...
while (TRUE) {
  ...
  weights_ccov <- rbind(weights_ccov, w_ccov)
  
  # (6) Graphical Elastic Net (GELNET) GMV
  S_cor      <- cor(data_win)
  ...
  Omega_ge   <- gelnet_fit$Theta
  w_gelnet   <- as.numeric(Omega_ge %*% oneN / as.numeric(t(oneN) %*% Omega_ge %*% oneN))
...
# Build equal‐weight matrix for turnover/metrics
...
metrics_ccov    <- q_metrics(weights_ccov)
...
TO_c   <- compute_TC(calc_TO(weights_ccov,  returns_oos))
...
# 5) Cov–Correlation GMV
cat("5) Cov–Correlation (Ledoit & Wolf) GMV:\n")
cat(sprintf(" Monthly Ret: %.4f%%  Var: %.4f  Sharpe: %.4f\n",
...
# 6) Graphical Elastic Net (GELNET, identity target)
cat("6) Graphical Elastic Net (identity target) GMV:\n")
cat(sprintf(" Monthly Ret: %.4f%%  Var: %.4f  Sharpe: %.4f\n",
