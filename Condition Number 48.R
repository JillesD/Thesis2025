...
df <- read.csv(
  "C:/Users/Jilles/OneDrive/Thesis/Data/Cleaned/48_Industry_Portfolios_excess_clean_rounded (1).csv",
...
while (TRUE) {
  ...
  S <- cov(Xw)
  Omega_samp    <- solve(S)
  cond_sample[j] <- kappa(Omega_samp, exact = TRUE)
  ...
  # (c) Ledoit–Wolf constant-correlation shrinkage
  Sigma_lw      <- covCor(Xw)
...
# 5) Summarize
res <- data.frame(
  Method = c("Glasso (Ω̂)", "Sample-Cov (S⁻¹)", "Ledoit–Wolf (Σ̂_LW⁻¹)"),
  Mean   = c(mean(cond_glasso), mean(cond_sample), mean(cond_lw)),
  SD     = c(  sd(cond_glasso),   sd(cond_sample),   sd(cond_lw))
)
