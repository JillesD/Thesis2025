# --------------------------------------
# Parallelized grid search for optimal λ for GELNET (correlation and covariance)
# with progress bars using doSNOW
# --------------------------------------
...
# Containers for average log‐likelihood for each λ (correlation and covariance)
...
# 4. Set up parallel backend using doSNOW (for progress bar support)
num_cores <- parallel::detectCores() - 1
cl <- makeCluster(num_cores, type = "SOCK")
registerDoSNOW(cl)

# 5a. Parallel loop over λ values for correlation‐based GELNET with progress bar
...
avg_loglik_cor <- foreach(l_idx = seq_len(n_lambda), 
                          ...
                          .options.snow = opts_cor) %dopar% {
                            ...
                            for (k in seq_len(n_eval)) {
                              ...
                              # Compute sample correlation & mean on the training window
                              S_cor <- cor(train_x)
                              mu    <- colMeans(train_x)
                              
                              # Fit GELNET (penalized precision) with alpha = 0.5
...
# 5b. Parallel loop over λ values for covariance‐based GELNET with progress bar
cat("\nGrid search: covariance‐based GELNET\n")
pb_cov <- txtProgressBar(min = 0, max = n_lambda, style = 3)
progress_cov <- function(n) setTxtProgressBar(pb_cov, n)
opts_cov <- list(progress = progress_cov)

avg_loglik_cov <- foreach(l_idx = seq_len(n_lambda), 
                          .combine = "c", 
                          .packages = c("GLassoElnetFast", "zoo"),
                          .options.snow = opts_cov) %dopar% {
                            lambda_val  <- lambda_values[l_idx]
                            loglik_vals <- numeric(n_eval)
...
# 6. Stop the cluster once both grid‐searches are done
stopCluster(cl)

# 7. Identify the optimal λ for each method
...
# 8. Print summary of λ vs. average log‐likelihood for both methods
...
# 9. (Optional) Plot average log‐likelihood vs. λ for both methods
plot(
  df_summary$Lambda, df_summary$AvgLogLik_Correlation, type = "b", col = "blue",
  xlab = "Lambda", ylab = "Average Log‐Likelihood", ylim = range(df_summary[, 2:3]),
...
# 10. Compute average sparsity (% zeros) at the chosen optimal λ for both methods
...
for (k in seq_len(n_eval)) {
  ...
  # (a) Correlation‐based GELNET at optimal λ
  S_cor <- cor(train_x)
  ...
  # (b) Covariance‐based GELNET at optimal λ
  S_cov <- cov(train_x)
